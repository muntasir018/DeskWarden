...
:: Save and disable Quick Edit Mode to prevent accidental pause on click
...
:: Restore Quick Edit Mode to original value
...
:find_python
set "PYTHON_EXE="
:: IMPORTANT: use python.exe (NOT pythonw.exe) so pip works correctly
...
copy /y "%SOURCE_SCRIPT%" "!INSTALL_DIR!\src\DeskWarden.py" >nul
if %errorLevel% neq 0 (
...
if exist "%~dp0assets\icon.png" (
    copy /y "%~dp0assets\icon.png" "!INSTALL_DIR!\assets\icon.png" >nul
)
if exist "%~dp0assets\deskwarden_full_icon.png" (
    copy /y "%~dp0assets\deskwarden_full_icon.png" "!INSTALL_DIR!\assets\deskwarden_full_icon.png" >nul
)
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$s.TargetPath='!LAUNCH_EXE!';" ^
  "$s.Arguments='\"!FINAL_SCRIPT!\" --open-control-panel';" ^
  "$s.WorkingDirectory='!INSTALL_DIR!';" ^
  "if($icon){$s.IconLocation=$icon};" ^
  "$s.WindowStyle=7; $s.Save();" ^
  "$s2=$WshShell.CreateShortcut('!STARTMENU_PATH!');" ^
  "$s2.TargetPath='!LAUNCH_EXE!';" ^
  "$s2.Arguments='\"!FINAL_SCRIPT!\" --open-control-panel';" ^
...
set "FINAL_SCRIPT=!INSTALL_DIR!\src\DeskWarden.py"
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Run" /v "DeskWarden" /t REG_SZ /d "\"!LAUNCH_EXE!\" \"!FINAL_SCRIPT!\"" /f >nul 2>&1
if %errorLevel% neq 0 (
...
call :ok "Auto-start on login registered."
goto :eof
